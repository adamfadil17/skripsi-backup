...
export async function PUT(req: NextRequest, { params }: { params: { workspaceId: string; userId: string } }) {
  try {
    ...
    if (isAdmin && targetUser.role === "SUPER_ADMIN") {
      return NextResponse.json({ message: "Admin cannot change Super Admin role" }, { status: 403 })
    }

    // Admin tidak boleh mempromosikan pengguna menjadi Super Admin
    if (isAdmin && newRole === "SUPER_ADMIN") {
      return NextResponse.json({ message: "Admin cannot promote to Super Admin" }, { status: 403 })
    }
    ...
    if (isSuperAdmin) {
      ...
      if (targetUser.role === "SUPER_ADMIN" && newRole !== "SUPER_ADMIN") {
        ...
        if (superAdminCount <= 1) {
          return NextResponse.json({ message: "At least one Super Admin must remain" }, { status: 403 })
        }
      }
      ...
      if (newRole !== "SUPER_ADMIN") {
        ...
        await pusherServer.trigger(`workspace-${workspaceId}`, "member-updated", updatedMember)
    ...
    if (isAdmin && targetUser.role === "ADMIN" && newRole === "MEMBER") {
      return NextResponse.json({ message: "Admin cannot demote another Admin to Member" }, { status: 403 })
    }

    // Member tidak bisa mengubah role siapa pun
    if (isMember) {
      return NextResponse.json({ message: "Members cannot change roles" }, { status: 403 })
    }

    return NextResponse.json({ message: "No valid role changes allowed" }, { status: 403 })
  } catch (error) {
    console.error(error)
...
export async function DELETE(req: NextRequest, { params }: { params: { workspaceId: string; userId: string } }) {
  try {
    ...
    if (currentUserRole?.role === "ADMIN" && targetUserRole?.role === "MEMBER") {
      ...
      // Trigger Pusher event for real-time updates
      await pusherServer.trigger(`workspace-${workspaceId}`, "member-removed", userId)
