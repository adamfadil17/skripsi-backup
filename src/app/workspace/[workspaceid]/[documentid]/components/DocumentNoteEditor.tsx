...
import EditorJS, { type ToolConstructable, type OutputData } from "@editorjs/editorjs"
...
const DocumentNoteEditor: React.FC<DocumentNoteEditorProps> = ({ workspaceId, documentId, modelResponse }) => {
  ...
  const saveDocument = useCallback(async () => {
    if (editorRef.current) {
      try {
        ...
        await axios.put(`/api/workspace/${workspaceId}/document/${documentId}/content/`, {
          content: outputData,
        })
      } catch (error) {
  ...
  const appendModelResponse = useCallback(
    async (response: any) => {
      ...
      try {
        ...
        // Prepare the model response as a new block
        // Assuming the response is text that should go into a paragraph block
        let newBlock

        // Check if response is already in EditorJS block format
        if (response && response.blocks) {
          // If it's already in EditorJS format, use its blocks
          newBlock = response.blocks
        } else {
          // Otherwise create a paragraph block with the response text
          newBlock = [
            {
              type: "paragraph",
              data: {
        ...
        // Combine existing blocks with new block
        ...
        // Save the document after appending
        saveDocument()
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (modelResponse && modelResponse !== prevModelResponseRef.current && editorRef.current) {
